...
import {
    ...
    _convert__api_ApiHomeResponse_TO_ApiHomeResponse,
} from './_converters';
import {
    _Home_GET_Home_url,
    _Home_GET_ApiHome_url,
} from './_url-builders';
export async function home(): Promise<string> {
    const _response: any = await _createHttp().get(_Home_GET_Home_url());
...
export async function apiHome(): Promise<ApiHomeResponse> {
    const _response: any = await _createHttp().get(_Home_GET_ApiHome_url());
